...
const validateCred = inputId => {
  let CredCardDigit = inputId.length - 2
  let checkSum = 0
  ...
  while (CredCardDigit != -1) {
    const checkFormula = ((inputId[CredCardDigit] * 2) < 9) ? inputId[CredCardDigit] * 2 : ((inputId[CredCardDigit] * 2) - 9)

    if (CredCardDigit % 2 === lenCheck){
      checkSum += checkFormula
      CredCardDigit --
    } else if (CredCardDigit % 2 === len2) {
      checkSum += inputId[CredCardDigit]
      CredCardDigit --
    }
  } if (CredCardDigit === -1) {
...
const findInvalidCards = (input, num) => {
  let counter = 1
  let companies = " Invalid card(s)";
